...
class TestGetMediaFileHeaders:
    ...
    @pytest.fixture
    def uploaded_media_id(self, client):
        """テスト用メディアをアップロードし、その ID を返す。"""
        r = client.post(
            "/media",
    ...
    def test_etag_header_present(self, client, uploaded_media_id):
        """ETag ヘッダが '"<hash>"' 形式で存在する。"""
        r = client.get(f"/media/{uploaded_media_id}/file")
        assert r.status_code == 200
    ...
    def test_cache_control_header_present(self, client, uploaded_media_id):
    ...
    def test_etag_matches_file_hash(self, client, db_engine, uploaded_media_id):
