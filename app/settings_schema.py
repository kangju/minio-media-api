"""Settings クラスと語彙ロード関数の定義。

このモジュールは Settings クラスのみを提供する。
settings インスタンス生成は config.py で行う（fail-fast）。
"""
...
class Settings(BaseSettings):
    ...
    """MinIO アクセスキー。環境変数 MINIO_ACCESS_KEY で必ず設定すること。"""

    minio_secret_key: str
    """MinIO シークレットキー。環境変数 MINIO_SECRET_KEY で必ず設定すること。"""
    ...
    # データベース設定
    database_url: str
    """PostgreSQL 接続 URL。環境変数 DATABASE_URL で必ず設定すること。"""
